    ...
    function finalizarEdicaoTaxaPoupanca() {
        ...
        let val = parseFloat(inp.value.replace(',', '.')) || 6.17;
        if (val < 0) val = 0;
        if (val > 50) val = 50;
    ...
    function calcularDadosPoupanca(labels, vInicial, aporteMensal, isCorrente) {
        const taxaAnual = isCorrente ? 0 : (parseFloat(document.getElementById('inputTaxaPoupanca')?.value) || 6.17) / 100;
    ...
    function executarPipelineCore() {
        ...
        let dataRenda = [Math.round(vInicial * taxaMensal)];
        let aporteVigentePerAno = {};

        baseDadosCalculados.anos = {};
        baseDadosCalculados.anos[0] = { investido: vInicial, juros: 0, total: vInicial, renda: vInicial * taxaMensal, aporteVigente: 0 };
        ...
        for (let m = 1; m <= totalMeses; m++) {
            ...
            if (modoAtual === 'completo' && avancadoAtivo) {
                ...
                if (retiradaAtiva && anoAtual >= anoInicioRetirada) {
                    saldoTotal = (saldoTotal * (1 + taxaMensal)) - valorRetiradaPretendida;
                } else {
    ...
    function atualizarMetricasExibidas() {
        ...
        if (anyPoupLine) {
            ...
            const diffP = poupancaAtivaPoupanca ? (() => {
                const taxaP = (parseFloat(document.getElementById('inputTaxaPoupanca')?.value) || 6.17) / 100;
    ...
    function setupPeriodArrows() {
        ...
        document.getElementById('btnIdadeUp')?.addEventListener('click', () => {
            ...
            const expectativa = parseInt(document.getElementById('inputExpectativaVida').value) || 85;
    ...
    function exportarExcel() {
        ...
        const sufixo = tipoPeriodo === 'anual' ? 'Ano' : 'Mês';
        const usaIpca = document.getElementById('chkInflacaoAporte')?.checked && modoAtual === 'completo';
        ...
        chaves.forEach(k => {
            const d = anos[k];
            const row = [k, d.total.toFixed(2), d.juros.toFixed(2), d.investido.toFixed(2), d.renda.toFixed(2), (d.renda/12).toFixed(2)];
    ...
    function exportarPDF() {
        ...
        const rendaMensalFinal = (finalDados.renda || 0) / 12;
        ...
        const tRows = chaves.map((k, i) => {
            ...
            return `<tr style="background:${bg};color:#d1d5db;">
                ...
                <td style="padding:5px 10px;text-align:right;color:#FFCC00;">${formatCurrency(d.renda/12)}</td>
            </tr>`;
        }).join('');
        ...
        <div class="resultado">
            ...
            <p style="color:#d1d5db;margin:0;">Renda mensal projetada (5% a.a.): <strong style="color:#FFCC00;">${formatCurrency(rendaMensalFinal)}/mês</strong></p>
        </div>
    ...
    function aplicarCisneNegro(dataTotal, labels) {
        ...
        // Encontrar índice do ano colapso
        let idxColapso = labels.findIndex(l => parseInt(l) >= anoColapso);

...
let FIIS_EXTRAS = {};  // Novos dados coletados a cada 15 dias (taxa admin, cotistas, DY, VPA, etc.)
let ACOES_EXTRAS = {};  // Novos dados coletados a cada 15 dias (CNPJ, ISIN, razão social, qtd ações, setor B3)
let DIVIDENDOS = [];
  ...
  try {
    ...
    if (Object.keys(FIIS_FUND).length > 0) {
      FIIS = Object.values(FIIS_FUND).map(f => {
        ...
        // Mescla dados extras (taxa de administração, cotistas, DY acumulado/médio, VPA, etc.)
        const extras = FIIS_EXTRAS[f.ticker] || {};
        return {
          t: f.ticker, n: cot.name || f.ticker,
          p: cot.price, v: cot.change || 0,
          // v7/v30: sem dado real pré-calculado — null exibe "—".
          ...
          // Dados extras (atualizados a cada 15 dias)
          taxa_administracao: extras.taxa_administracao || null,
          taxa_gestao: extras.taxa_gestao || null,
          taxa_performance: extras.taxa_performance || null,
          cotistas: extras.cotistas || null,
          dy_3m_acum: extras.dy_3m_acum || null,
          dy_6m_acum: extras.dy_6m_acum || null,
          dy_12m_acum: extras.dy_12m_acum || null,
          dy_3m_media: extras.dy_3m_media || null,
          dy_6m_media: extras.dy_6m_media || null,
          dy_12m_media: extras.dy_12m_media || null,
          vpa: extras.vpa || null,
          p_vpa: extras.p_vpa || null,
          patrimonio_liquido: extras.patrimonio_liquido || null,
          gestora: extras.gestora || null,
          administrador: extras.administrador || null,
          cnpj: extras.cnpj || null,
          data_constituicao: extras.data_constituicao || null,
          cotas_emitidas: extras.cotas_emitidas || null,
          liquidez_media: extras.liquidez_media || null,
          fallback: cot.fallback || false,
          stale:    cot.stale    || false,
          _source:  cot._source,
        };
      ...
      FIIS = (json.fiis||[]).map(d => {
        ...
        const extras = FIIS_EXTRAS[d.ticker] || {};
        return {
          t: d.ticker, n: cot.name || d.ticker,
          p: cot.price || 0, v: +(cot.change||0).toFixed(2),
          v7: null, v30: null,
    ...
    // ── FIIs Extras (atualizado a cada 15 dias) ────────────────────────────────
    try {
      const er = await fetch('data/fiis_extras.json');
      if (er.ok) {
        const ej = await er.json();
        FIIS_EXTRAS = (ej.fundos || {});
      }
    } catch(e) {}

    // ── Ações Extras (atualizado a cada 15 dias) ────────────────────────────────
    try {
      const ar = await fetch('data/acoes_extras.json');
      if (ar.ok) {
        const aj = await ar.json();
        ACOES_EXTRAS = (aj.acoes || {});
      }
    } catch(e) {}
